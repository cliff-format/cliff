...
- **Whether a key may be quoted.** The grammar decides it: `key = name`
  ([specification §5.5](../spec/cliff-1.1.0.md)) and a `name` cannot contain a
  quote, so `"title":` is a syntax error and not a spelling this guide could
  recommend either way. A tolerant parser may accept a quoted key as the documented
  relaxation of [Appendix C.2.7](../spec/cliff-1.1.0.md) and report the repair,
  which is a conformance option, not a recommendation.
